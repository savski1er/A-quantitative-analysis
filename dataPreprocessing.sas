...
/*Step 3*/
/*Merging Patient-level primary care continuity score and Usual provider continuity index*/
...
data aaistp; set ais.aais; keep maskssn atten_phynpi amb; amb=1; run;
data datind; set aaistp; proc sort; by atten_phynpi; run;
data aaistp; set aaistp; by atten_phynpi;
    retain phy;
      if first.atten_phynpi then phy=amb;
      else phy=phy+amb;
      if last.atten_phynpi;
      if phy>1;
run;

data aaist; set aaist aaistp; run;
...
drop pecmorb2;
drop pecmorb3;
drop mod_code atten_phyid oper_phyid othoper_phyid oper_phynpi othoper_phynpi ecmorb:;
run;
...
drop drg;
rename ndrg=drg;
nzipcode = input(zipcode, 15.);
drop zipcode;
rename nzipcode=zipcode;
nweekday = input(weekday, 3.);
drop weekday;
rename nweekday=weekday;
nptcounty = input(ptcounty, 6.);
drop ptcounty;
rename nptcounty=ptcounty;
nfac_region = input(fac_region, 6.);
drop fac_region;
rename nfac_region=fac_region;
nadm_prior = input(adm_prior, 3.);
drop adm_prior;
rename nadm_prior=adm_prior;
natten_phynpi = input(atten_phynpi, 30.);
drop atten_phynpi;
rename natten_phynpi=atten_phynpi;
...
data dat.datindx4; set sec.datindx4; drop phy wphy pcp1 phy1 tx tx1 tx2;run;

data datind; set dat.datindx4; keep maskssn atten_phynpi tx; tx=1; run;
data datind; set datind; proc sort; by atten_phynpi; run;
data datind; set datind; by atten_phynpi;
    retain phy;
      if first.atten_phynpi then phy=tx;
      else phy=phy+tx;
      if last.atten_phynpi;
      if phy>1;
run;
...
/*Merging step*/
data datind1; set datind; keep maskssn atten_phynpi phy;run;
proc sql;
create table iaist as select a.*,b.* 
from dat.datindx4 a left join datind1 b
on a. atten_phynpi= b. atten_phynpi;
quit;

/*continuity of care sensitive to changes in the total number of visits and 
in their distribution across different providers. It ranges from 0 to 1, 
where 0 occurs when each visit is to a different provider and 1 occurs when 
all visits are to one provider*/
data iaist;set iaist;if phy=. then phy=0;run;

data iaist;set iaist;
coc_r = (sum((pcp)*(pcp))-sum(pcp))/(sum(pcp)*(sum(pcp)-1));
run;
data iaist; set iaist;
if coc_r=. then coc_r=0;
run;

data iaist; set iaist;
if coc_r=1 and er=1 then tx1=1;
else tx1=0;
run;

data iaist; set iaist;
if er=0 and coc_r=1 then ptgrp=1; /*no er and yes ih and yes coc_r*/
if er=0 and coc_r=0 then ptgrp=2; /*no er and yes ih and no coc_r*/
if er=1 and coc_r=1 then ptgrp=3; /*yes er and yes ih and yes coc_r*/
if er=1 and coc_r=0 then ptgrp=4; /*yes er and yes ih and no coc_r*/
run;



/*Joining patient regional incom*/
proc import datafile = '/Users/macuser/Desktop/aisproject/dat/incom.csv'
 out = work.incom
 dbms = csv
 replace;
run;
...
/*Merging step*/
data incom; set incom; keep fac_region incq1 incq2 incq3 incq4 incamed;run;
proc sql;
create table iaist as select a.*,b.* 
from iaist a left join incom b
on a. fac_region= b. fac_region;
quit;

data dat.datindx4;set iaist;run;
data dat.datind;set iaist;drop atten_phynpi pcp er ih incq1 incq2 incq3 incq4 sys_recid;run;
...
proc import datafile = 'C:\Users\Vassiki Sanogo\Desktop\aisproject\dat\datindx4.csv'
 out = work.datindx4
 dbms = csv
 replace;
run;

data datinds;set datindx4;keep maskssn tchgs tchger tchgamb los loser losamb;run;

proc sql;
create table dat.datindv as select a.*,b.* 
from datindv a left join datinds b
on a. maskssn= b. maskssn;
quit;


proc export data=dat.datindx4 dbms=csv
outfile="/Users/macuser/Desktop/aisproject/dat/datindx.csv"
replace;
run;


proc export data=dat.datind dbms=csv
outfile="/Users/macuser/Desktop/aisproject/dat/datind.csv"
replace;
run;

proc freq data=datind; tables qtr zipcode ptcounty weekday drg;
ods pdf file= "/Users/macuser/Desktop/aisproject/dat/table.pdf" style=vasstables;
run;

/*Step 12*/
/*Variable harmonization*/

%Macro dummy (input,output);

data &output; set &input; 
 
*categorize ethnicity*;
	if ethnicity = 1 then ethnic = 1;
	else ethnic=0;

*categorize gender*;
	if sex =1 then sexc = 1; * Male *;
	else if sex =2 then sexc = 0; * Female *;
	
*categorize race*;
	if race = 1 then race_black = 1; * Black *;
	if race = 2 then race_white = 1; * White *;
	if race =3 then race_other = 1; * All Others *;
	
*categorize payer*;
	if payer =0 then payer_noinsur = 1; * No Insurance *;
	if payer =1 then payer_gov = 1; * Government Insurance *;
	if payer =2 then payer_privinsur = 1; * Private Insurance *;
	if payer =3 then payer_othinsur = 1; * Other Insurance *;

*reformating admsrc*;
if admsrc=1 then admsrc_home=1; *Non-Health Care Facility Point of Origin*; 
if admsrc=2 then admsrc_clinic=1; *Clinic or Physician’s Office*;
if admsrc=3 then admsrc_hosp=1; *Transfer from a Hospital *;
if admsrc=4 then admsrc_nursing=1; *Transfer from a Skilled Nursing Facility (SNF) or Intermediate Care Facility (ICF)*;
if admsrc=5 then admsrc_othhcf=1; *Transfer from another health care facility*;
if admsrc=6 then admsrc_lawenf=1; *Court/Law Enforcement*;
if admsrc=7 then admsrc_unkwn=1; *Information Not Available*;
if admsrc=8 then admsrc_duh=1; *Transfer from one distinct unit of the hospital to another distinct unit of the same hospital resulting in a separate claim*;
if admsrc=9 then admsrc_amb=1; *Transfer from an Ambulatory Surgery Center*;
if admsrc=10 then admsrc_hospise=1; *Transfer from a hospice facility & under a hospice plan of care or enrolled in a hospice program Codes Required for Newborn Admissions*;
if admsrc=11 then admsrc_inshosp=1; *Born inside this hospital*;
if admsrc=12 then admsrc_outhosp=1; *Born outside this hospital*;

npoa_prin_diag = input(poa_prin_diag, 6.);
	drop poa_prin_diag;
if condpr =0 then condpra=1; *Not present at the time that the order for inpatient admission occurs*;
if condpr =1 then condprb=1; *Present at time that order for inpatient admission occurs*;
if condpr =2 then condprc=1; *Documentation is insufficient to determine if condition is present on admission*;
if condpr =3 then condprd=1; *Provider is unable to clinically determine whether the condition was present on admission or not*;
if condpr =4 then condpre=1; *The condition is exempt from POA reporting*;

if condprs =0 then condprsa=1; *Not present at the time that the order for inpatient admission occurs*;
if condprs =1 then condprsb=1; *Present at time that order for inpatient admission occurs*;
if condprs =2 then condprsc=1; *Documentation is insufficient to determine if condition is present on admission*;
if condprs =3 then condprsd=1; *Provider is unable to clinically determine whether the condition was present on admission or not*;
if condprs =4 then condprse=1; *The condition is exempt from POA reporting*;

if exinju =1 then exinjua=1; *Not present at the time that the order for inpatient admission occurs*;
if exinju =2 then exinjub=1; *Present at time that order for inpatient admission occurs*;
if exinju =3 then exinjuc=1; *Documentation is insufficient to determine if condition is present on admission*;
if exinju =4 then exinjud=1; *Provider is unable to clinically determine whether the condition was present on admission or not*;
if exinju =5 then exinjue=1; *The condition is exempt from POA reporting*;
if exinju=0 then exinjuf=1;

if cond=1 then conda=1; *Patient received treatment in this facility’s emergency department.*;
if cond=2 then condb=1; *Not admitted through this facility’s emergency department.*;
if cond=3 then condc=1; *Not reported*;

/* formarting poa_ecmorb1*/
if pecmorb =1 then pecmorba=1;
if pecmorb =2 then pecmorbb=1;
if pecmorb =3 then pecmorbc=1;
if pecmorb =4 then pecmorbd=1;
if pecmorb =5 then pecmorbe=1;
if pecmorb=0 then pecmorbf=1;

/*adm time and employees shift time*/
if admtime=1 then admtimea=1;
if admtime=2 then admtimeb=1; /*switch 1*/
if admtime=3 then admtimec=1;
if admtime=4 then admtimed=1; /*switch 2*/
if admtime=5 then admtimee=1;
if admtime=6 then admtimef=1; /*switch 3*/
if admtime=7 then admtimeg=1;; /*Unknown*/

/*dis time and employees shift time*/
if distime=1 then distimea=1;
if distime=2 then distimeb=1; /*switch 1*/
if distime=3 then distimec=1;
if distime=4 then distimed=1; /*switch 2*/
if distime=5 then distimee=1;
if distime=6 then distimef=1; /*switch 3*/
if distime=7 then distimeg=1;; /*Unknown*/

/*edhr_arr and employees shift time*/
if edhrarr=1 then edhrarra=1;
if edhrarr=2 then edhrarrb=1; /*switch 1*/
if edhrarr=3 then edhrarrc=1;
if edhrarr=4 then edhrarrd=1; /*switch 2*/
if edhrarr=5 then edhrarre=1;
if edhrarr=6 then edhrarrf=1; /*switch 3*/
if edhrarr=7 then edhrarrg=1;; /*Unknown*/

/*Type of sverity*/
if sevr=1 then sevra=1; /*ACUTE ISCHEMIC STROKE W USE OF THROMBOLYTIC AGENT W MCC*/
if sevr=2 then sevrb=1; /*ACUTE ISCHEMIC STROKE W USE OF THROMBOLYTIC AGENT W CC*/
if sevr=3 then sevrc=1; /*ACUTE ISCHEMIC STROKE W USE OF THROMBOLYTIC AGENT W/O CC/MCC*/
if sevr=4 then sevrd=1/*Other*/;

*reformating distat*;
if ptstat=1 then ptstata=1;
if ptstat=2 then ptstatb=1;
if ptstat=3 then ptstatc=1;
if ptstat=4 then ptstatd=1;
if ptstat=5 then ptstate=1;
if ptstat=6 then ptstatf=1;
if ptstat=7 then ptstatg=1;
if ptstat=8 then ptstath=1;
if ptstat=9 then ptstati=1;
if ptstat=10 then ptstatj=1;

*reformating fac_region*;
if fac_region=1 then fac_regiona=1; *Local Health Council 1*; 
if fac_region=2 then fac_regionb=1; *Local Health Council 2*;
if fac_region=3 then fac_regionc=1; *Local Health Council 3*;
if fac_region=4 then fac_regiond=1; *Local Health Council 4*;
if fac_region=5 then fac_regione=1; *Local Health Council 5*;
if fac_region=6 then fac_regionf=1; *Local Health Council 6*;
if fac_region=7 then fac_regiong=1; *Local Health Council 7*;
if fac_region=8 then fac_regionh=1; *Local Health Council 8*;
if fac_region=9 then fac_regioni=1; *Local Health Council 9*;
if fac_region=10 then fac_regionj=1; *Local Health Council 10*;
if fac_region=11 then fac_regionk=1; *Local Health Council 11*;

drop ethnicity race sex payer admsrc npoa_prin_diag condpr condprs exinju cond pecmorb admtime
distime edhrarr sevr ptstat fac_region zipcode weekday ptcounty mcare_nbr qtr maskssn faclnbr agegrp overall;

run;

%mend dummy;

%dummy (dat.datind,datind); *obs 186564*;
